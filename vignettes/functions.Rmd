...
```{r message=F}
setwd("D:/JHU/Spring19/601.749ComparativeGenomics/Project/bioSeqGen/data/try")
pcent0_21mer <- read.table('0pcent21mer.fa',stringsAsFactors = F )
library(dplyr)
...
for(i in seq(1,n)){
      splitKmer <- strsplit(tempKmers,'',fixed = T)[[1]]
      tempSameKmer <- 0
      for(j in seq(1,kmerLen)){
            ...
            tempSameKmer <- tempSameKmer + sum(mutatedKmers %in% tempKmers)
      }
