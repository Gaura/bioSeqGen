...
depth <- 30
nreads <- 1e5*30/5000
startPos <- sample(1e5,nreads)
reads <- NULL
for(i in 1:nreads){
      if(startPos[i] + 4999 > 1e5){
            reads <- c(reads,substr(ecoli1L,startPos[i],1e5))
      }
      else{
            reads <- c(reads,substr(ecoli1L,startPos[i],startPos[i] + 4999))
      }
...
for(i in 1:5){
      ...
      p5m21copy$freq[simKmersIndices[1]] <- sum(p5m21copy$freq[simKmersIndices])
      p5m21copy <- p5m21copy[-simKmersIndices[-1],]
}
```
