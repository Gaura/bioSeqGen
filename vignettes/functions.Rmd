...
```
Now make a function to iteratively select adjoining clusters and move forward (this is not efficient)
```{r}
library(dplyr)
...
```
Separate self cluster distance from cross cluster distance
```{r}
library(dplyr)
dists <- sort(unique(kmers10kE05X30Df$refDist))
...
```
After checking within a cluster, checking between clusters
```{r}
library(dplyr)
#add refDist, connected and index columns before inputing into function and df sorted according to refDist
sapply(1:(length(dists)-1), function(x){
...
dist1 <- which(distMatKm10kE05X30D67 == 1)
#later add the previous rows but for here since it is the first 2 clusters don't need to add
dimMatR <- nrow(distMatKm10kE05X30D67)
